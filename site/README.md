...
cargo install --git https://github.com/chevdor/tera-cli --locked
```

> We rely on the `chevdor/tera-cli` build because the crates.io `tera-cli` does
> not ship the `--include-path` / `--env-only` flags that our build script uses.
...
| `npm run build`    | Cleans and recompiles all templates using `tera`, then copies `public/` into `build/dist`. |
