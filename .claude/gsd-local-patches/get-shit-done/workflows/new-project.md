...
**Check for global defaults** at `~/.planning/defaults.json`. If the file exists, offer to use saved defaults:

```
AskUserQuestion([
  {
    question: "Use your saved default settings? (from ~/.planning/defaults.json)",
    header: "Defaults",
    multiSelect: false,
    options: [
...
If "Yes": read `~/.planning/defaults.json`, use those values for config.json, and skip directly to **Commit config.json** below.

If "No" or `~/.planning/defaults.json` doesn't exist: proceed with the questions below.
