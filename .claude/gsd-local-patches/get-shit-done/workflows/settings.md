<purpose>
Interactive configuration of GSD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.planning/defaults.json) for future projects.
</purpose>
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.planning/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.planning/defaults.json`:

```bash
mkdir -p ~/.planning
```

Write `~/.planning/defaults.json` with:
```json
{
  "mode": <current>,
  "depth": <current>,
...
- [ ] User offered to save as global defaults (~/.planning/defaults.json)
